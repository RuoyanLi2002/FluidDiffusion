...
def create_batch_edge_index(data):
    batch_edge_index = []
    node_offset = 0

    for data in data.to_data_list():
        temp_x = data.x
        edge_index = create_edge_index(temp_x)

        edge_index = edge_index + node_offset
        batch_edge_index.append(edge_index)
        node_offset += temp_x.size(0)
...
class GNNDenoiser(nn.Module):
    ...
    def forward(self, x, timestep, batch_index, data):
        edge_index = create_batch_edge_index(data)
...
class DDPM(nn.Module):
    ...
    def forward(self, data):
        ...
        # forward diffusion
        perturbed_x, _ = self.forward_diffusion(x_zeros, t, batch_index)
        
        # backward diffusion
        pred_x0 = self.model(perturbed_x, t, batch_index, data)

        return perturbed_x, x_zeros, pred_x0
    ...
    def denoise_at_t(self, x_t, timestep, t, edge_index):
        ...
        x0_pred = self.model(x_t, edge_index, timestep, batch_index)
        ...
        x_t_minus_1 = sqrt_alpha * x0_pred + sqrt_one_minus_alpha_bar * z
        
        return x_t_minus_1

    def sample(self, x_t):
        # x_t = torch.randn((x_t.shape[0], x_t.shape[1])).to(self.device)
        edge_index = create_edge_index(x_t).to(self.device)
